...
namespace Homework_4._5
{
   internal class ClassFor1DArray
   {
      ...
      public static void ComparisonNegative(int a, int b, int c)
      {
         ...
         Console.WriteLine("Минимальное количество отрицательных элементов в массивах {0}, {1}, {2} равно: {3}", name[0], name[1], name[2], min);
         ...
         while (counter < arr.Length)
         {
            ...
            if (arr[counter] == min)
            {
               Console.WriteLine("В массиве {0} минимальное количество отрицательных элементов: ", name[counter]);
            }
